...
static bool
log_attempt(int log, struct crypt_device *cd, uid_t uid, bool success)
{
    char msg[4096] = {};
    char *dev = NULL;
    int r = 0;

    dev = audit_encode_nv_string("dev", crypt_get_device_name(cd), 0);
    if (!dev)
        return false;

    r = snprintf(msg, sizeof(msg),
                 "cmd=clevis-luks-udisks2 sauid=%d uid=%d uuid=%s %s",
                 audit_getloginuid(), uid, crypt_get_uuid(cd), dev);
    free(dev);
    if (r < 0 || r == sizeof(msg))
        return false;
...
main(int argc, char *const argv[])
{
    ...
    for (pkt_t req = {}, jwe = {}, key = {}; ; key = (pkt_t) {}) {
        ...
        if (!log_attempt(log, cd, uid, key.used > 0))
